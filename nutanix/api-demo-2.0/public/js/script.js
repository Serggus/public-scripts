$(document).ready(function(){
    ...
    $( 'input#submit-go').on( 'click', function(e) {
        ...
        else
        {
            ...
            var cluster_address = $( '#cvm-address' ).val();
            var cluster_port = $( '#cvm-port' ).val();
            var username = $( '#cluster-username' ).val();
            var password = $( '#cluster-password' ).val();
            var timeout = $( '#cluster-timeout' ).val();

            /* Submit the request via AJAX */
            request = $.ajax({
                url: "/ajax/buildDashboard",
                type: "post",
                dataType: "json",
                data: { cluster_address: cluster_address, cluster_port: cluster_port, username: username, password: password, timeout: timeout }
            });
            ...
            request.success( function(data) {
                if( data.result == 'ok' )
                {
                    ...
                    $( 'span#ssd_graph').html( '<img src="/img/charts/' + data.ssdGraph + '">' );
                    $( 'span#hdd_graph').html( '<img src="/img/charts/' + data.hddGraph + '">' );
            ...
            request.fail(function ( jqXHR, textStatus, errorThrown )
            {
                ...
                $( '#clusterDetails' ).slideUp( 300 );
                $( 'span#cluster-error').html( 'Unfortunately an error occurred while processing the request.  Status: ' + textStatus + ', Error Thrown: ' + errorThrown );
                $( '#clusterError' ).slideDown( 300 );
            });
